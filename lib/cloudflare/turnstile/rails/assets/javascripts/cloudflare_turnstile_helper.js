...
(function() {
   ...
   * pendingCount - how many placeholders we're observing but haven't mounted.
   *                Maintained alongside an `_cfPending` flag on each element so
   *                the same placeholder can't be counted twice. Exists purely
   *                so the always-installed form-interaction listeners can bail
   *                out in constant time: `pointerover` fires on every element
   *                boundary the cursor crosses, and walking up to closest()
   *                each time — forever, long after every widget has mounted —
   *                is work we can skip with an integer compare. Only ever
   *                over-counts (a pending placeholder removed from the DOM is
   *                never decremented), which is the safe direction: we do
   *                needless work rather than miss a trigger.
   */
  var PENDING_SELECTOR = '.cf-turnstile:not([data-turnstile-rendered])',
    RESERVED_ATTR = 'data-turnstile-reserved',
   ...
   * value back out if we're the one who set it.
   *
   * Ownership is recorded in a `data-turnstile-reserved` attribute rather than
   * a JavaScript property on the element, so that it lives in the same place
   * as the thing it describes. Turbo snapshots a page by cloning it: attributes
   * and inline styles survive that round trip, JS properties don't. With the
   * marker on the node, a restored placeholder still carries both halves of
   * the reservation and we can release it on mount. With the marker beside the
   * node, the style would come back without any record that it was ours, and
   * the `min-height` would be stuck for good — which matters most to the very
   * apps the release is protecting, the ones using an invisible sitekey that
   * forgot `config.reserve_space = false`.
   *
   * Setting this attribute doesn't feed our own MutationObserver, whose
   * attributeFilter covers style/class/hidden/open only.
   *
   * @param {Element} el
   * @returns {void}
   */
  function applyReservation(el) {
    ...
    if (el.hasAttribute(RESERVED_ATTR)) {
      return;
    }
    ...
    el.setAttribute(RESERVED_ATTR, '');
  }

  /**
   * Drop a reservation this script made, once the real widget has taken the
   * element's place and is sizing it for us. No-ops for elements not carrying
   * our marker, so an author's own `min-height` is never touched — including
   * one that happens to be exactly the height we would have reserved.
   *
   ...
   *
   * The widget id that render() hands back is kept on the element, because
   * turnstile.remove() needs it to dismiss the widget again (see
   * resetForCache). It's a JavaScript property rather than an attribute
   * deliberately: the id belongs to this page's live Turnstile instance and
   * must not survive into a cached page snapshot.
   *
  ...
  function mount(el) {
    ...
    ensureLoaded(function() {
      ...
      try {
        el._cfWidgetId = window.turnstile.render(el);
  ...
  function setupLazyTriggers() {
    ...
    /*
     * These two stay installed for the life of the page, because a form (and
     * its widget) can arrive at any time via Turbo. Tearing them down once
     * nothing is pending would need re-arming on every later insertion, so we
     * keep them and make the no-work case a single integer compare instead.
     */
  ...
  /**
   * `turbo:before-cache` / `turbolinks:before-cache` handler.
   *
   * Turbo caches a page by cloning its DOM, and a back/forward navigation
   * restores that clone without asking the server for anything. Everything a
   * rendered widget leaves behind is part of the clone: the iframe, our
   * `data-turnstile-rendered` marker, and — the part that actually bites — the
   * hidden `cf-turnstile-response` input holding a token. Restore the page and
   * the user faces a widget whose token has already been spent or has since
   * expired, while the marker and the iframe both tell every mount path to
   * leave it alone. Submitting the form sends that dead token, verification
   * fails, and there's no way out short of a full reload.
   *
   * So we put each placeholder back the way the server sent it, just before the
   * clone is taken: dismiss the widget, empty the container, drop the marker.
   * The restored copy reads as a fresh placeholder, mounts through the normal
   * path, and gets a new token. The reservation goes back on as well, so the
   * restored page holds the widget's space instead of jumping when the new
   * iframe lands.
   *
   * Auto-rendered widgets are covered too, hence the isAlreadyMounted() test
   * rather than a marker-only selector: under `render=auto` Cloudflare mounts
   * widgets without going through mount(), so there is no marker — but there is
   * still a stale token in the snapshot.
   *
   * `pendingCount` is deliberately left alone. These nodes are about to be
   * discarded with the rest of the body, and the placeholders in the incoming
   * page are counted by observeOne() when dispatch() picks them up.
   *
   * Only registered in lazy and eager mode. In passive mode the host app owns
   * its widgets, and tearing them down behind its back would be worse than
   * leaving a stale one in place.
   *
   * @returns {void}
   */
  function resetForCache() {
    var els, i, el, id;

    els = document.querySelectorAll('.cf-turnstile');
    ...
    for (i = 0; i < els.length; i++) {
      ...
      /*
       * turnstile.remove() takes the widget id that render() returned, which is
       * why mount() keeps it on the element; it also releases Cloudflare's own
       * bookkeeping, which emptying the container alone would leak. When there
       * is no id — an auto-rendered widget never went through mount() — or
       * remove() objects, clearing the container is enough for our purposes:
       * with no iframe and no hidden response input left, the placeholder reads
       * as un-mounted again.
       */
      id = el._cfWidgetId;
      ...
      if (id && window.turnstile && typeof window.turnstile.remove === 'function') {
        try {
          ...
          // eslint-disable-next-line no-console
          console.warn('cloudflare-turnstile-rails: turnstile.remove failed', e);
        }
      }

      el._cfWidgetId = null;
      // textContent rather than innerHTML: we only ever clear, never write markup.
      el.textContent = '';
      delete el.dataset.turnstileRendered;
      applyReservation(el);
    }
  }
   ...
   * gestured.
   *
   * Also hooks `before-cache`, the other half of the Turbo page-cache story:
   * dispatch() re-discovers placeholders on the way in, resetForCache() makes
   * sure the snapshot on the way out contains placeholders worth
   * re-discovering rather than spent widgets.
   *
  ...
  function setupTurboHooks() {
    ...
    document.addEventListener('turbolinks:before-cache', resetForCache);
  }
